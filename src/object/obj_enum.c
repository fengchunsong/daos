	       ...
	       daos_size_t iod_size, int type)
{
	...
	priv_off = rec->rec_recx.rx_idx;
	priv_size = rec->rec_recx.rx_nr;
	if (priv_off + priv_size != curr_off) /* not continuous */
		return true;
	...
	if (arg->ec_cell_sz > 0 &&
	    (priv_off + priv_size - 1) / arg->ec_cell_sz !=
	       ...
	       daos_size_t iod_size, struct obj_enum_rec **new_rec)
{
	...
	/* First check if it is continuous record */
	if (arg->last_type == OBJ_ITER_RECX && type == OBJ_ITER_RECX && arg->ec_cell_sz > 0) {
		rec = iovs[arg->sgl_idx].iov_buf + iovs[arg->sgl_idx].iov_len - sizeof(*rec);
		*new_rec = rec;
